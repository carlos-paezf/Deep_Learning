...
def classify(document):
    label = {-1: 'Sin sentimiento', 0: 'Neutral', 1: 'Positivo', 2: 'Negativo'}
...
def sqlite_entry(path, document, y):
    ...
    cursor = conn.cursor()
    cursor.execute("INSERT INTO tweets_db (tweet, sentiment, date) VALUES (?, ?, DATETIME('now'))", (document, y))
    conn.commit()
    conn.close()
...
def sqlite_select(path):
    conn = sqlite3.connect(path)
    cursor = conn.cursor()
    cursor.execute('SELECT tweet, sentiment, date FROM tweets_db')
